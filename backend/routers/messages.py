...
async def send_wa_template(
    ...
    template_displays = {
        ...
        "welcome_textech1": f"אז אנחנו יוצאים לדרך! 🏁 שיהיה לנו המון בהצלחה!\n\n{v[0]}, את/ה תקבל/י הודעות WhatsApp תחת השם Tax Tech.\n\nאת/ה תקבל/י הודעה על אישור ייצוג לביטוח לאומי שתצטרך/י לאשר.\n\nדרך טקסטק תוכל/י להיות במעקב ולהתעדכן בסטטוס התיק שלך בכל רגע! 😊\n\nלכל שאלה מקצועית תוכל/י לפנות לצ'אט המקצועי, לכל שאלה שירותית תוכל/י לפנות אליי.\n\nתודה רבה על שיתוף הפעולה!\n\nקבוצת יש לי זכות | שד' העצמאות 91, קרית אתא",
    ...
    try:
        ...
        if template_name == "free" and all_vars:
            ...
            is_within_24h = False
            if last_inbound:
                last_inbound_time = last_inbound.created_at
                if last_inbound_time.tzinfo is None:
                    last_inbound_time = last_inbound_time.replace(tzinfo=timezone.utc)
                is_within_24h = (now_utc - last_inbound_time) < timedelta(hours=24)
            if is_within_24h:
